...
public class Rb extends JButton {
	...
	protected void paintComponent(Graphics g) { // 그래픽스 (임포트java.awt)안의 페인트 컴포넌트 함수 재정의
		...
		graphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON); // 안티에일리어싱 사용(그래픽
																										// 사용중 계단현상방지)
		if (getModel().isArmed()) {
			graphics.setColor(backColor.darker());
		} // 버튼(getModel() 클릭하는동안 색상을 어둡게 설정
		else if (getModel().isRollover()) {
			graphics.setColor(backColor.brighter());
		} // 마우스가 올라가면 밝게 설정
		else {
